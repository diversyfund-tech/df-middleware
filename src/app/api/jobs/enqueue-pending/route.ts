...
/**
 * Enqueue pending webhook events for processing
 * POST /api/jobs/enqueue-pending
 */
	...
	try {
		...
		for (const event of pendingEvents) {
			try {
				...
				console.error(`[enqueue-pending] Failed to enqueue event ${event.id}:`, error);
			}
		}
		...
		console.error("[enqueue-pending] Error:", error);
